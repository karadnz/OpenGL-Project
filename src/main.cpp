...
int main(int arc,char** argv)
{
    ...
    graf::Scene scene;
    //scene.addModel("LightTextureCube.json");

    // Add multiple cubes to the scene
    std::vector<std::string> modelFiles = {"LightTextureCube.json", "LightTextureCube.json", "LightTextureCube.json"};
    std::vector<glm::vec3> positions = {glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(2.0f, 0.0f, 0.0f), glm::vec3(-2.0f, 0.0f, 0.0f)};
    ...
    {
        if(key==GLFW_KEY_LEFT); // scene.removeModel(&model) ;  //        transform.moveForward();
